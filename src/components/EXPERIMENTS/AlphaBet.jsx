...
const renderGame = (state) => _setStateCopy && _setStateCopy(state)
const renderGameOver = () => console.log('GAME OVER!')
const noop = () => { }
...
var _setStateCopy = null
game$.subscribe(renderGame, noop, renderGameOver)

const AlphaBet = () => {
    const [state, setState] = useState(null)

    if (!_setStateCopy) _setStateCopy = setState

    if (!state) return null

    return (
        <div>
            <header>{ `Score: ${state.score}, Level: ${state.level}` }</header>
            {
                state.letters.map(l => '&nbsp'.repeat(l.yPos) + l.letter + '<br/>')
            }
            {
